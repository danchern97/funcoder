...
class FuncoderInjectedTestsPrompt(MakeTestPrompt):
    ...
    _test_store = PyCtx[list[_InjectedTestsStore]]("funcoder_injected_tests_store")
...
class FuncoderInjectedStdioPrompt(MakeTestPrompt):
    """Provide stdin / stdout as tests via PyCtx injection. Must specify test
    cases via `.inject(...)` directly within a parent call."""

    _test_store = PyCtx[list[_InjectedStdioStore]]("funcoder_injected_stdio_store")
...
class FuncoderInjectedExprPrompt(MakeTestPrompt):
    """Provide one-line expression tests via PyCtx injection. Specify test
    cases via `.inject(...)` directly within a parent call."""

    _test_store = PyCtx[list[_InjectedExprStore]]("funcoder_injected_expr_store")
