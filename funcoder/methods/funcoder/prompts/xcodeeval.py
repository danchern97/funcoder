...
class FuncoderXCodeEvalDividePrompt(GenOncePrompt):
...
```"""
    ...
    get_few_shots = [
        {
            "role": "system",
            "content": """You are a programming copilot, you can solve a CodeForces or ICPC problem by writing Python functions. Your task is to:
- You need to write a Python function `main()` that read input from STDIN and print the answer to STDOUT, please DONNOT print redundant characters.
...
class FuncoderXCodeEvalConquerPrompt(GenOncePrompt):
...
class FuncoderXCodeEvalFuncCallPrompt(MakeTestPrompt):
...
class FuncoderXCodeEvalUnitTestPrompt(MakeTestPrompt):
