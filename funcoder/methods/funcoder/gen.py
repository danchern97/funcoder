...
from .dfs_2pass import FunCoderDfs2Pass
from .gen_once import GenOncePrompt, funcoder_gen_once, gen_collect_program
from .make_test import MakeTestPrompt, TestType, funcoder_make_test
from .runner import RunnerCaseResult, funcoder_runner


class FuncoderGen(CodeGenMethod):
    ...
    async def gen(
        ...
        ctx.log.in_scope(f"funcoder[{func.name}]")
    ...
    async def _pass_2_test(
        ...
        ctx.log.in_scope(f"funcoder_pass_2_test")
        _sj = CodeGenJournalist(ctx, "funcoder_pass_2_test", (ancestors, pass_func, pass_descendants))
